...
void computeNormal(int i, int j,int w,unsigned char* c,vector<Point> &normal) {
	float fi=float(i);
	float fj=float(i);
	
	float p1[3] = {fi,float(c[ ((j-1)+(w*i)) ]),fj-1};
	float p2[3] = {fi,float(c[ ((j+1)+(w*i)) ]),fj+1};
	float p3[3] = {fi-1,float(c[ ((j)+(w*(i-1))) ]) ,fj};
	float p4[3] = {fi+1,float(c[ (j+(w*(i+1))) ]),fj};
	float v1[3] = {0,0,0};
	sub(p2,p1,v1);
	float v2[3] = {0,0,0};
	sub(p4,p3,v2);
...
// 	Build the vertex arrays
	...
	//preparar pontos
	drawGrid(th,tw,p,imageData,normals,texPoints);//prencher vetor de pontos de acordo com o tamanho da figura
